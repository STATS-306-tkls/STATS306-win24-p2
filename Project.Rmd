...
summary(titles_basics)  # General summary statistics

# For quantitative columns:
titles_basics %>% summarise(across(where(is.numeric), ~list(mean = mean(., na.rm = TRUE), sd = sd(., na.rm = TRUE))))

# For categorical columns:
titles_basics %>% count(column_name)  # Replace `column_name` with actual column names
ggplot(titles_basics) +
...
different_names_count <- titles_basics %>%
...
```{r}
alliteration_count <- titles_basics %>%
