...
ui <- fluidPage(
    ...
    sidebarLayout(
        sidebarPanel(
            ...
            uiOutput("actor_dropdown"), # This will be the UI for our dynamic dropdown
...
server <- function(input, output, session) {
    ...
    observeEvent(input$search_button, {
        ...
        output$actor_dropdown <- renderUI({
            selectInput("actor_select", "Select an Actor:", choices = setNames(actors_in_titles$primaryName, actors_in_titles$nconst))
        })
    ...
    observeEvent(input$next_button, {
        ...
        if (nrow(current_path) < 6) {
            current_path <- rbind(current_path, name_basics %>% filter(nconst == current_actor))
            paths(current_path)
            connected_actors <- get_connected_actors(current_actor)
            
            if ("nm0000102" %in% connected_actors$nconst) {
                game_status("Congratulations! You found Kevin Bacon in " %s+% nrow(current_path) %s+% " steps!")
                return()
            }
            
            output$actor_dropdown <- renderUI({
                selectInput("actor_select", "Select an Actor:", choices = setNames(connected_actors$primaryName, connected_actors$nconst))
            })
...
get_connected_actors <- function(current_actor) {
    # Logic to find connected actors based on the selected actor's nconst
    # This is a placeholder function and should be implemented as per your datasets
    return(connected_actors_dataframe)
}
