...
### Part (a)
Consider a function $f(x) = -\sqrt{x} + x$. Give a number $n$, calculate $f(x)$ for any $x$ which satisfies $x \leq n$ using for loop, save the results as a vector. Let $n = 500$, print the fist 5 elements of your final results. The function `Sys.time()` can help you calculate the time. Use `Sys.sleep(0.1)` inside for loop to suspend the execution of each iteration 0.1 second. Print the running time 
```{r}
#f <- function(x) {
#   return(-sqrt(x) + x)
# }
...
# run_time
```
Modify your above code using parallization. Also use `Sys.sleep(0.1)` inside for loop to suspend the execution of each iteration 0.1 second. Consider to use the tools of `foreach` and `doParallel`. You can use 2 cores for this question. Print the running time. Compare with the running time in (a), what have you observed?

This running time is much faster, taking half the time as in part a. 
```{r}
# cl <- makeCluster(2)
...
# 
# para <- foreach(x = 1:n, .combine = c) %dopar% {
#   Sys.sleep(0.1)
#   f(x)
# }
# 
...
```
### Part (c):
Complete the same task above. This time use `parSapply()` instead. You do not need to specify the sleep time in this question.
```{r}
# cl <- makeCluster(2)
# start_time3 <- Sys.time()
# 
# running <- parSapply(cl, 1:n, f)
# 
# end_time3 <- Sys.time()
# 
